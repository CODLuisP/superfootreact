  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-900 tracking-tight">Gestionar Catálogo</h1>
        ...
        <div className="flex items-center gap-2.5">
          <div className="inline-flex rounded-xl border border-gray-300 bg-white shadow-2xs overflow-hidden divide-x divide-gray-200">
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-semibold text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-wait transition"
              title="Exportar CSV"
            >
          ...
          <button type="button" onClick={() => setImportOpen(true)} className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl border border-gray-300 bg-white text-xs font-semibold text-gray-700 hover:bg-gray-50 shadow-2xs transition">
            <Upload className="h-4 w-4 text-gray-500" />
            <span>Importar Excel</span>
          </button>
          <button type="button" onClick={onNavigateToCargar} className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold shadow-md shadow-emerald-600/20 transition">
      ...
      {exportMsg && (
        <div className={`mb-6 rounded-xl p-3 text-xs font-semibold ${exportMsg.startsWith('✓') ? 'bg-emerald-50 border border-emerald-200 text-emerald-800' : 'bg-red-50 border border-red-200 text-red-700'}`}>
          {exportMsg}
        </div>
      )}
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:items-center">
          <div className="relative flex-1">
            ...
            <input
              ...
              type="text"
              placeholder="Buscar por nombre o código de barras… (búsqueda instantánea, indexada)"
              value={searchInput}
          ...
          <div className="inline-flex rounded-xl bg-gray-100 p-0.5 border border-gray-200 shrink-0">
            <button type="button" onClick={() => setViewMode('table')} className={`p-1.5 rounded-lg transition ${viewMode === 'table' ? 'bg-white text-emerald-700 shadow-xs' : 'text-gray-500 hover:text-gray-800'}`} title="Vista en tabla"><List className="h-4 w-4" /></button>
            <button type="button" onClick={() => setViewMode('grid')} className={`p-1.5 rounded-lg transition ${viewMode === 'grid' ? 'bg-white text-emerald-700 shadow-xs' : 'text-gray-500 hover:text-gray-800'}`} title="Vista en cuadrícula"><Grid className="h-4 w-4" /></button>
          </div>
        </div>
      ...
      {loadError && <div className="mb-6 rounded-xl bg-red-50 border border-red-200 p-3 text-xs text-red-700">{loadError}</div>}

      {/* Product Content */}
      {!loading && items.length === 0 ? (
        <div className="bg-white rounded-2xl border border-gray-200 p-12 text-center">
          <div className="h-14 w-14 rounded-2xl bg-gray-100 text-gray-400 flex items-center justify-center mx-auto mb-3"><PackageX className="h-8 w-8" /></div>
            ...
            <table className="w-full text-left text-xs text-gray-600">
              <thead className="bg-gray-50/80 text-[11px] uppercase font-bold text-gray-500 border-b border-gray-200 tracking-wider">
                <tr>
                  ...
                  <th className="px-4 py-3.5">Producto / Nombre</th>
                  <th className="px-4 py-3.5">Código de barras</th>
                ...
                {items.map((p) => (
                  <tr key={p.id} className="hover:bg-gray-50/80 transition group">
                    <td className="px-4 py-3">
                      <div className="h-12 w-12 rounded-xl bg-gray-100 border border-gray-200 overflow-hidden shrink-0 flex items-center justify-center">
                        {p.image ? <img src={p.image} alt={p.name} loading="lazy" className="h-full w-full object-cover" /> : <span className="text-[10px] text-gray-400 font-medium text-center px-1">Sin foto</span>}
                      </div>
                    </td>
                    <td className="px-4 py-3 max-w-xs">
                      <div className="font-bold text-gray-900 text-xs sm:text-sm truncate">{p.name}</div>
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap"><BarcodeBadge code={p.code} /></td>
                    <td className="px-4 py-3 whitespace-nowrap text-[11px] text-gray-500">{formatDate(p.createdAt)}</td>
                    <td className="px-4 py-3 text-right whitespace-nowrap">
                      <div className="flex items-center justify-end gap-1.5">
                        <button type="button" onClick={() => setEditingProduct(p)} className="p-1.5 rounded-lg text-gray-600 hover:bg-emerald-50 hover:text-emerald-700 transition" title="Editar producto"><Edit2 className="h-4 w-4" /></button>
                        <button type="button" onClick={() => setProductToDelete(p)} className="p-1.5 rounded-lg text-gray-400 hover:bg-red-50 hover:text-red-600 transition" title="Eliminar producto"><Trash2 className="h-4 w-4" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
          {items.map((p) => (
            <div key={p.id} className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-xs hover:shadow-md transition-shadow flex flex-col justify-between">
              <div>
                <div className="relative h-44 w-full bg-gray-50 border-b border-gray-100 flex items-center justify-center overflow-hidden">
                  {p.image ? (
                    ...
                    <div className="text-center p-4 text-gray-400 text-xs"><PackageX className="h-8 w-8 mx-auto mb-1 opacity-50" /><span>Sin imagen registrada</span></div>
                  )}
                  <div className="absolute top-2.5 right-2.5">
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-bold bg-emerald-600/90 text-white shadow-xs backdrop-blur-xs"><CheckCircle2 className="h-3 w-3" /> Aprobado</span>
                  </div>
                </div>
                <div className="p-4 space-y-2.5">
                  <h3 className="font-bold text-gray-900 text-sm line-clamp-2 leading-snug">{p.name}</h3>
                  <div className="pt-1"><BarcodeBadge code={p.code} /></div>
                </div>
              </div>
              <div className="px-4 py-3 bg-gray-50/80 border-t border-gray-100 flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  <button type="button" onClick={() => setEditingProduct(p)} className="p-1.5 rounded-lg text-gray-600 hover:bg-emerald-100 hover:text-emerald-700 transition" title="Editar"><Edit2 className="h-3.5 w-3.5" /></button>
                  <button type="button" onClick={() => setProductToDelete(p)} className="p-1.5 rounded-lg text-gray-400 hover:bg-red-100 hover:text-red-600 transition" title="Eliminar"><Trash2 className="h-3.5 w-3.5" /></button>
                </div>
              </div>
            </div>
          ))}
